...
class EfficientNet(nn.Module):
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        ...
        out_dict = dict()
        layer = 0
        for ix, layer_i in enumerate(self.features):
            x = layer_i(x)
            if ix in self.layers_of_interest:
                out_dict[str(layer)] = x
                layer += 1
...
def efficientnet_b0(
    ...
    layers_of_interest = [2, 3, 5, 8]
    return _efficientnet(
        inverted_residual_setting, kwargs.pop("dropout", 0.2), last_channel, weights, progress,
        layers_of_interest=layers_of_interest, **kwargs
    )
...
class EfficientNetB0FPN(torch.nn.Module):
    def __init__(self, weights=None):
        super().__init__()
        
        self.body = efficientnet_b0(weights=weights)
    ...
    def forward(self, x):
        feat_dict = self.body(x)
        out = self.fpn(feat_dict)
        return out


class EfficientNetB0(nn.Module):
    def __init__(self, d_model=128, weights=None):
        super().__init__()
        self.body = efficientnet_b0(weights=weights)

        feat_dims = [24, 40, 112, 1280]
        input_proj_list = []
        for ix in range(len(feat_dims)):
            input_proj_list.append(nn.Sequential(
                nn.Conv2d(feat_dims[ix], d_model, kernel_size=1),
    ...
    def forward(self, x):
        feat_dict = self.body(x)
        output_dict = {}
        for ix, v in enumerate(feat_dict.values()):
            output_dict[str(ix)] = self.input_proj[ix](v)

        return output_dict


if __name__ == '__main__':
    img = torch.randn(4, 3, 400, 880)
    enb0 = efficientnet_b0()
    enb0fpn = EfficientNetB0FPN(weights=None)
    res18 = torchvision.models.detection.backbone_utils.resnet_fpn_backbone(backbone_name='resnet50',
                                                                            weights=None,
                                                                            trainable_layers=5)

    for k, v in enumerate(enb0(img).values()):
        print(v.shape)
    print(enb0fpn(img)['0'].shape)
