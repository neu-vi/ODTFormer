# This file is derived from [Pixloc](https://github.com/cvg/pixloc/blob/master/pixloc/pixlib/geometry/wrappers.py).
...
class Pose(TensorWrapper):
    ...
    @staticmethod
    def from_4x4mat(T: torch.Tensor):
...
class Obb3D(TensorWrapper):
    ...
    def add_padding(self, max_elts: int = 100) -> "Obb3D":
        """
        ...
        pad_elt = -1 * self._data.new_ones(self._data.shape[-1])
        if num_to_pad > 0:
    ...
    def remove_padding(self) -> List["Obb3D"]:
        ...
        """
        assert self._data.ndim <= 3, "higher than order 3 remove_padding not supported yet"

        if self._data.ndim == 1:
            return self  # Nothing to be done in this case.

        # All -1's denotes a pad element.
        pad_elt = (-1 * self._data.new_ones(self._data.shape[-1])).unsqueeze(-2)
...
if __name__ == '__main__':
    cam_meta = torch.tensor([1, 2, 3, 4, 5, 6])[None, None, None, :].repeat(4, 2, 4, 1)
    camera_l = Camera(cam_meta)

    ref_points = torch.randn(4, 2, 4, 20, 3)

    # print(camera_l.project(ref_points)[0].shape)
    proj_points = camera_l.project(ref_points)[0]
    mask = torch.bitwise_not(camera_l.in_image(proj_points))[..., None].repeat(1, 1, 1, 1, 2)
    print(proj_points.shape, mask.shape)
    print(torch.masked_fill(proj_points, mask, -1.).shape)
