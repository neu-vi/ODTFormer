...
#             self.count = torch.zeros(1, device=x['loss'].device)
#         else:
...
#             tensor = torch.distributed.all_reduce(tensor, torch.distributed.ReduceOp.SUM, async_op=False)
#             return tensor
        
#         self.data = _all_reduce_(self.data)
#         self.count = _all_reduce_(self.count)
